...
rule plot_network:
    ...
    output:
        ...
        today=RESULTS
        + "maps/elec_s{simpl}_{clusters}_off-{offgrid}_l{ll}_{opts}_{sector_opts}_{planning_horizons}-today.pdf",
    threads: 2
    resources:
        mem_mb=10000,
    benchmark:
        (
            BENCHMARKS
            + "plot_network/elec_s{simpl}_{clusters}_off-{offgrid}_l{ll}_{opts}_{sector_opts}_{planning_horizons}"
        )
    conda:
